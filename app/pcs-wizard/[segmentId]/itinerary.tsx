...
import { Bed, ChevronRight, MapPin, Plus, Trash2, X } from 'lucide-react-native';
...
export default function ItineraryBuilderScreen() {
  ...
  const handleAddStop = () => {
    ...
    // Sort stops by date? For now, append.
    updateDraft({
      userPlan: {
  ...
  const handleNext = () => {
    ...
    // Gap Logic
    const departure = new Date(currentDraft.dates.projectedDeparture); // From previous segment/origin
    const arrival = new Date(currentDraft.dates.projectedArrival); // To destination
    const durationMs = arrival.getTime() - departure.getTime();
    const durationDays = durationMs / ONE_DAY_MS;
    const hasStops = (currentDraft.userPlan.stops?.length || 0) > 0;

    if (durationDays > 1 && !hasStops) {
      Alert.alert(
        'Missing Overnight Stop',
        'Your travel time is greater than 1 day. Where will you be sleeping?',
        [
          { text: 'Add Stop', onPress: () => setIsModalVisible(true) },
          { text: 'Continue Anyway', style: 'cancel', onPress: () => router.push(`/pcs-wizard/${currentDraft.id}/review`) }
        ]
      );
    } else {
      router.push(`/pcs-wizard/${currentDraft.id}/review`);
    }
  };
  ...
  return (
    <View className="flex-1 bg-slate-50 dark:bg-slate-950">
      <ScreenHeader
        ...
        subtitle="Step 3 of 4: Stops"
        leftAction={{ icon: X, onPress: () => router.back() }} // Should this be back or close? Wizard flow implies back to previous step.
      />
