    ...
    return (
        ...
        >
            <View className="flex-row justify-between items-start mb-2">
                <View>
                    <Text className="text-slate-500 dark:text-slate-400 text-[9px] font-bold uppercase tracking-widest">My Shortlist</Text>
            ...
            <View className="flex-row gap-4 h-12">
                <Pressable
                    ...
                    className="flex-1 bg-rose-50 dark:bg-rose-500/20 rounded-lg flex flex-col items-center justify-center border border-rose-100/50 dark:border-rose-500/30 h-full"
                >
                    <Star size={14} color="#F43F5E" fill="#F43F5E" className="text-rose-500 mb-0.5" />
                    <Text className="text-[10px] font-bold text-rose-700 dark:text-rose-300 leading-none">{superLiked}</Text>
                </Pressable>
                <Pressable
                    hitSlop={{ top: 15, bottom: 15, left: 10, right: 10 }}
                    style={({ pressed }) => [{ opacity: pressed ? 0.6 : 1 }]}
                    className="flex-1 bg-blue-50 dark:bg-blue-500/20 rounded-lg flex flex-col items-center justify-center border border-blue-100/50 dark:border-blue-500/30 h-full"
                >
                    <ThumbsUp size={14} color="#3B82F6" className="text-blue-500 mb-0.5" />
                    <Text className="text-[10px] font-bold text-blue-700 dark:text-blue-300 leading-none">{liked}</Text>
                </Pressable>
                <Pressable
                    hitSlop={{ top: 15, bottom: 15, left: 10, right: 10 }}
                    style={({ pressed }) => [{ opacity: pressed ? 0.6 : 1 }]}
                    className="flex-1 bg-slate-100 dark:bg-slate-700/50 rounded-lg flex flex-col items-center justify-center border border-slate-200/50 dark:border-slate-600/50 h-full"
                >
                    <X size={14} color={isDark ? "#94A3B8" : "#64748B"} className="text-slate-500 mb-0.5" />
                    <Text className="text-[10px] font-bold text-slate-600 dark:text-slate-300 leading-none">{passed}</Text>
                </Pressable>
            </View>
        </GlassView>
    );
