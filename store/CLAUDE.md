...
**NEVER:** Use Zustand `persist` without handling state versioning
**ALWAYS:** Implement a `migrate` function and `version` number when using persisted stores to avoid caching crashes on schema updates
